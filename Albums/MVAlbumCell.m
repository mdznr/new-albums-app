...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
	...
	if ( self )
	{
		...
		albumView_.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
		albumView_.drawBlock = ^(UIView *view, CGContextRef ctx)
		{
			if ( !cell.album ) return;
			...
			if ( [cell.album.releaseDate compare:[NSDate date]] == NSOrderedDescending )
			{
				...
				CGRect labelRect = CGRectMake(cell.frame.size.width - marginRight - labelWidth, 20.5, labelWidth, 18);
		...
		spinnerView_ = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
...
- (void)setAlbum:(MVAlbum *)album
{
  if ( album == album_ ) return;
	...
	if ( !self.album.albumType )
		...
		text = [NSString stringWithFormat:NSLocalizedString(@"Hide %@", @"Hide %@ Label"), self.album.albumType];
...
- (void)layoutSubviews
{
	...
	hideAlbumLabelViewFrame.origin.y = roundf((self.frame.size.height - hideAlbumLabelViewFrame.size.height) / 2);
...
- (void)mvPanGestureRecognizer:(UIPanGestureRecognizer *)panGesture
{
	...
	if(panGesture.state == UIGestureRecognizerStateEnded)
	{
		...
		CAMediaTimingFunction *timingFunction = [CAMediaTimingFunction functionWithControlPoints:0.61 :1.5 :0.78 :1];
		
		CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"position.x"];
		animation.fromValue = [NSNumber numberWithFloat:self.albumView.frame.origin.x + frame.size.width / 2];
		animation.toValue = [NSNumber numberWithFloat:frame.origin.x + frame.size.width / 2];
		animation.timingFunction = timingFunction;
		animation.duration = 0.3;
		animation.delegate = self;
		[self.albumView.layer addAnimation:animation forKey:nil];
	...
	else
	{
		...
		hideAlbumLabelViewFrame.origin.x = kMVAlbumControlStartMargin + (finalX - kMVAlbumControlStartMargin) * (MIN(translate.x, treshholdX) / treshholdX);
		self.hideAlbumLabelView.frame = hideAlbumLabelViewFrame;
		...
		CGRect hideArtistLabelViewFrame = self.hideArtistLabelView.frame;
		float beginX = self.frame.size.width - hideArtistLabelViewFrame.size.width - kMVAlbumControlStartMargin;
		finalX = self.frame.size.width - (hideArtistLabelViewFrame.origin.y + kMVAlbumControlEndMarginFromY) - hideArtistLabelViewFrame.size.width;
		treshholdX = (hideArtistLabelViewFrame.origin.y + kMVAlbumControlEndMarginFromY) * 2 + hideArtistLabelViewFrame.size.width;
